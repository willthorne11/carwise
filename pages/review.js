...
import { canUseReview, incrementReview, getRemainingReviews } from '../lib/usage'
...
export default function Review() {
  ...
  useEffect(() => {
    setRemaining(getRemainingReviews())
  ...
  const runReview = async () => {
    if (!canUseReview()) { router.push('/unlock'); return }
    if (!make || !model) { setError('Please enter at least the make and model.'); return }
    setLoading(true)
    setError('')
    try {
      const res = await fetch('/api/analyse', {
        ...
        body: JSON.stringify({
          type: 'review',
          data: { make, model, year, mileage, price, fuel, concerns }
        })
      })
        ...
        <div className={styles.wrap}>
          ...
          <div className={styles.flags}>
            {result.flags?.map((f, i) => (
              <span key={i} className={`chip chip-${f.type}`}>{f.text}</span>
            ))}
          </div>

          {/* MOT */}
          <div className={styles.drop}>
            <div className={styles.dropHeader} onClick={() => toggleDrop('mot')}>
              <div className={styles.dropLeft}>
                <span className={styles.dropTitle}>MOT history</span>
                <span className={`${styles.badge} ${styles.badgeGood}`}>Check gov.uk</span>
              </div>
              <svg className={`${styles.arrow} ${openDrops.mot ? styles.open : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </div>
            {openDrops.mot && (
              <div className={styles.dropBody}>
                <p className={styles.dropNote}>Enter the reg on <a href="https://www.check-mot.service.gov.uk" target="_blank" rel="noreferrer" style={{color: 'var(--accent)'}}>gov.uk/check-mot-history</a> for the full official MOT record. It's free and shows every test, advisory and failure.</p>
              </div>
            )}
          </div>
          ...
          {/* Insurance */}
          <div className={styles.drop}>
            <div className={styles.dropHeader} onClick={() => toggleDrop('ins')}>
              <div className={styles.dropLeft}>
                <span className={styles.dropTitle}>Insurance estimate</span>
                <span className={`${styles.badge} ${styles.badgeGood}`}>Group {result.insurance_group_number}</span>
              </div>
              <svg className={`${styles.arrow} ${openDrops.ins ? styles.open : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </div>
            {openDrops.ins && (
              <div className={styles.dropBody}>
                <div className={styles.insGrid}>
                  {result.insurance_groups && Object.entries({
                    'Age 17–21': result.insurance_groups['17_21'],
                    'Age 22–25': result.insurance_groups['22_25'],
                    'Age 26–35': result.insurance_groups['26_35'],
                    'Age 35+': result.insurance_groups['35_plus'],
                  }).map(([label, val]) => (
                    <div key={label} className={styles.insCard}>
                      <div className={styles.insLabel}>{label}</div>
                      <div className={styles.insVal}>{val}</div>
                    </div>
                  ))}
                </div>
          ...
          {/* Road tax */}
          <div className={styles.drop}>
            <div className={styles.dropHeader} onClick={() => toggleDrop('tax')}>
              <div className={styles.dropLeft}>
                <span className={styles.dropTitle}>Road tax</span>
                <span className={`${styles.badge} ${styles.badgeGood}`}>£{result.road_tax_annual}/yr</span>
              </div>
              <svg className={`${styles.arrow} ${openDrops.tax ? styles.open : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </div>
            {openDrops.tax && (
              <div className={styles.dropBody}>
          ...
          {/* What owners say */}
          <div className={styles.drop}>
            <div className={styles.dropHeader} onClick={() => toggleDrop('owners')}>
              <div className={styles.dropLeft}>
                <span className={styles.dropTitle}>What owners say</span>
                <span className={`${styles.badge} ${styles.badgeGood}`}>{result.owner_rating} / 5</span>
              </div>
              <svg className={`${styles.arrow} ${openDrops.owners ? styles.open : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </div>
            {openDrops.owners && (
              <div className={styles.dropBody}>
                ...
                {[
                  { label: 'Reliability', val: result.owner_reliability },
                  { label: 'Running costs', val: result.owner_running_costs },
                  { label: 'Comfort', val: result.owner_comfort },
                  { label: 'Practicality', val: result.owner_practicality },
                ].map(b => (
                  ...
                  <div key={i} className={styles.reviewItem}>
                    <div className={styles.reviewTop}>
                      ...
                      <div className={styles.stars}>
                        {[1,2,3,4,5].map(s => <div key={s} className={`${styles.star} ${s <= r.stars ? styles.starOn : ''}`}></div>)}
                      </div>
                    </div>
          ...
          {/* Depreciation */}
          <div className={styles.drop}>
            <div className={styles.dropHeader} onClick={() => toggleDrop('dep')}>
              <div className={styles.dropLeft}>
                <span className={styles.dropTitle}>Depreciation</span>
                <span className={`${styles.badge} ${styles.badgeWarn}`}>~20%/yr</span>
              </div>
              <svg className={`${styles.arrow} ${openDrops.dep ? styles.open : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </div>
            {openDrops.dep && (
              <div className={styles.dropBody}>
          ...
          {/* Viewing checklist */}
          <div className={styles.drop}>
            <div className={styles.dropHeader} onClick={() => toggleDrop('check')}>
              <div className={styles.dropLeft}>
                <span className={styles.dropTitle}>Viewing checklist</span>
                <span className={`${styles.badge} ${doneCount === totalChecks ? styles.badgeGood : styles.badgeNeutral}`}>{doneCount} of {totalChecks} done</span>
              </div>
              <svg className={`${styles.arrow} ${openDrops.check ? styles.open : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </div>
            {openDrops.check && (
              <div className={styles.dropBody}>
                  ...
                  {[
                    ...
                    <div key={item.id} className={`${styles.checkItem} ${checked[item.id] ? styles.checkDone : ''}`} onClick={() => toggleCheck(item.id)}>
                      ...
                      <div>
                        ...
                        {item.warn && <div className={styles.checkWarn}>{item.warn}</div>}
                      </div>
                    </div>
                  ))}
                </div>
                {result.checklist_specific?.length > 0 && (
                  <div className={styles.checkSection}>
                    <div className={styles.checkSectionTitle}>{make} {model} specific</div>
          ...
          {result.alternatives?.map((alt, i) => (
            <div key={i} className={styles.altCard}>
              <div>
                ...
                <div className={styles.altWhy}>{alt.why}</div>
              </div>
          ...
          <p className={styles.searchesLeft}>
            {remaining > 0 ? `${remaining} free reviews remaining` : 'No free reviews left'}
          </p>
  ...
  return (
    <div>
      ...
      <div className={styles.wrap}>
        ...
        <div className="row2">
          <div className="field"><label>Make</label><input type="text" value={make} onChange={e => setMake(e.target.value)} placeholder="e.g. Ford" /></div>
          <div className="field"><label>Model</label><input type="text" value={model} onChange={e => setModel(e.target.value)} placeholder="e.g. Fiesta" /></div>
        </div>
        <div className="row2">
          <div className="field"><label>Year</label><input type="number" value={year} onChange={e => setYear(e.target.value)} placeholder="e.g. 2018" min="1990" max="2025" /></div>
          <div className="field"><label>Mileage</label><input type="number" value={mileage} onChange={e => setMileage(e.target.value)} placeholder="e.g. 45000" /></div>
        </div>
        <div className="row2">
          <div className="field"><label>Asking price (£)</label><input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="e.g. 8500" /></div>
          <div className="field">
        ...
        <p className={styles.searchesLeft} style={{marginTop: '0.75rem'}}>
          {remaining > 0 ? `${remaining} free reviews remaining` : 'No free reviews left — '}
          {remaining <= 0 && <a href="/unlock" style={{color: 'var(--accent)'}}>unlock unlimited for £7.99</a>}
        </p>
      </div>
    </div>
  )
